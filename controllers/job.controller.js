...
const sendSalaryMailForStaff = async (req, res, next) => {
	const { month1, year1 } = req.body;
	// const d = new Date();
	...
	try {
		...
		for (let i = 0; i < list_final.length; i++) {
            ...
            <th>Tổng nhận</th>          
				...
				let totalListDetailMoney = 0;
				for (k = 0; k < listDetail.length; k++) {
					totalListDetailMoney += parseFloat(listDetail[k].amount);
					const trText = `<tr>
                                    <td>${k + 1}</td>
				...
				<p>Thành tiền: ${(totalListDetailMoney + listClass[j].bonus).toLocaleString(
					'vi-VN',
					{
						style: 'currency',
						currency: 'VND',
...
const calculatePaymentStudent = async (req, resq, next) => {
	...
	`;
		...
		return resq.status(200).json({ message: 'Thành công' });
	} catch (err) {
		console.log(err);
...
const sendPaymentMailForStudent = async (req, res, next) => {
	const { month1, year1 } = req.body;
	// const d = new Date();
	const month = parseInt(month1);
	const year = parseInt(year1);
...
ORDER BY users.user_id`;
		...
		for (let i = 0; i < list_mail.length; i++) {
            ...
            <th>Tên lớp</th>
            <th>Học phí / buổi</th>
            <th>Tổng số buổi tháng ${current_month}</th>
            <th>Phí nợ (tháng ${last_month})</th>
            <th>Phí thừa (tháng ${last_month})</th>
            <th>Thành tiền</th>
          </tr>
            `;
			...
			const mailOptions = {
				...
				subject: `Thông báo học phí tháng ${current_month}`,
				text: '',
				html: `<html>
			            <head>
                        <style>
			...
			<body>
				<div>
					...
					<p>Xoài Academy gửi gia đình và em học phí tháng ${current_month}:</p>
					${tablehtmlContent}
...
module.exports = {
	sendSalaryMailForStaff,
	calculatePaymentStudent,
	sendPaymentMailForStudent,
};
